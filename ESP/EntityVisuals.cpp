...
std::shared_ptr<Task> UpdatePlayers = std::make_shared<Task>(10, [] {
    if (!EngineInstance)
        return;
    EngineInstance->UpdatePlayers();
    });

void DrawEntityVisuals()
{
    if (!EngineInstance)
        return;
    ...
    for (auto entity : EngineInstance->GetActors())
    {
        ...
        Vector3 campos = Vector3(EngineInstance->GetCameraCache().POV.Location.X,
            EngineInstance->GetCameraCache().POV.Location.Y,
            EngineInstance->GetCameraCache().POV.Location.Z);
