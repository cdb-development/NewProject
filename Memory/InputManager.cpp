...
bool Keyboard::InitKeyboard()
{
	...
	win_logon_pid = MemoryManager.GetPidFromName("winlogon.exe");
	if (Winver > 22000)
	{
		auto pids = MemoryManager.GetPidListFromName("csrss.exe");
		for (size_t i = 0; i < pids.size(); i++)
		{
			auto pid = pids[i];
			uintptr_t tmp = VMMDLL_ProcessGetModuleBaseU(MemoryManager.vHandle, pid, (LPSTR)"win32ksgd.sys");
			uintptr_t g_session_global_slots = tmp + 0x3110;
			uintptr_t user_session_state = MemoryManager.Read<uintptr_t>(MemoryManager.Read<uintptr_t>(MemoryManager.Read<uintptr_t>(g_session_global_slots, pid), pid), pid);
	...
	else
	{
		...
		bool result = VMMDLL_Map_GetEATU(MemoryManager.vHandle, MemoryManager.GetPidFromName("winlogon.exe") | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, (LPSTR)"win32kbase.sys", &eat_map);
		if (!result)
			return false;
...
void Keyboard::UpdateKeys()
{
	...
	VMMDLL_MemReadEx(MemoryManager.vHandle, win_logon_pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, gafAsyncKeyStateExport, (PBYTE)&state_bitmap, 64, NULL, VMMDLL_FLAG_NOCACHE);
